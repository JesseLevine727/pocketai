#include "../../build/m5_iterate_c3_v1/generated/ops.c"
#include "../../runtime/m5_scalar/uniform_affine.c.inc"
...
int pa_scalar_uniform_probe(int candidate, void *work, uint32_t capacity,
                             const int32_t *input, double scale, const int32_t *bias,
                             const double *scales, const double *bias_double,
                             uint32_t count, int16_t *output, uint32_t *state) {
  pa_m5_workspace workspace;
  pa_m5_workspace_open(&workspace, work, capacity);
  workspace.used = 17;
  pa_m5_backend backend = {state, 0, probe_sfpu};
  int status = candidate ? pa_scalar_uniform_affine(&backend, &workspace, input,
                            scale, bias, count, output) :
                          affine_row(&backend, &workspace, input, scales, bias_double,
