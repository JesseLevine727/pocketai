#!/usr/bin/env bash
# Experimental consumer-refined SRAM port, isolated from accepted 2x RTL.
set -euo pipefail
cd "$(dirname "$0")/.."
mapping=${1:?usage: m6_assemble_1x_candidate.sh build/m6_mapping new_build_m6_output}
...
timeout --signal=TERM --kill-after=20s 180s docker run --rm --name "$container" \
  ...
  'read_json /input/mapped.json; read_verilog -sv /input/wrappers.sv /rtl/pa_m6_sram_1x_candidate.sv; read_verilog -sv -lib /sram/sram22_512x32m4w8.v; hierarchy -check -top pa_m6_portable; proc; opt; memory_map; opt; opt_expr -undriven; opt_clean; check -assert; write_verilog -noattr portable.v; write_json portable.json; stat' \
  > "$output/run.log" 2>&1
sha256sum "$mapping/mapped.json" "$mapping/wrappers.sv" \
  asic/m6/rtl/pa_m6_sram_1x_candidate.sv scripts/m6_assemble_1x_candidate.sh \
