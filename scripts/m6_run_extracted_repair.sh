...
cp asic/m6/extracted_repair_entry.py asic/m6/extracted_repair.tcl "asic/m6/$sdc" "$output/${tag}_flow/"
sha256sum asic/m6/extracted_repair_entry.py asic/m6/extracted_repair.tcl "asic/m6/$sdc" \
...
timeout --signal=TERM --kill-after=20s "${M6_TIMEOUT:-600s}" docker run --rm --name "$container" \
  ...
  -c SIGNOFF_SDC_FILE=/m6_flow/$sdc -c RUN_POST_GRT_DESIGN_REPAIR=true \
