"""Read-only evidence audit for the bounded 0.1-token/s pursuit."""
import argparse
import json
import subprocess
import sys
...
FINAL = 'build/m5_tenth_reciprocal_v1'
REPRO = 'build/m5_tenth_repro_v1'
CANDIDATES = dict(round='build/m5_tenth_round_v2', factor='build/m5_tenth_factor_v1', reciprocal=FINAL)
DECISIONS = {name: 'RETAIN' for name in CANDIDATES}
...
def manifest_paths():
    sources = set()
    for pattern in ('runtime/m5_tenth/*', 'tests/m5_tenth/*.py', 'tests/m5_tenth/*.c',
                    'scripts/*m5_tenth*', 'docs/M5_TENTH*.md'):
        sources.update(str(p.relative_to(ROOT)) for p in ROOT.glob(pattern) if p.is_file())
    sources.update(('scripts/m5_search_sources.py', 'scripts/build_m5_search.sh',
                    'scripts/audit_m5_search.py', 'tests/m5_iterate/performance_policy.json',
                    'zynq/m5_iterate_profile.py', 'zynq/m5_opt_profile.py', 'zynq/m5_run.py',
                    'zynq/m5_fast_complete.py', 'zynq/m5_opt_release_check.py'))
    artifacts = {'docs/m5_search_evidence.json'}
...
def validate(evidence, check_files=True):
    ...
    old = json.loads(read('docs/m5_search_evidence.json'))
    require(evidence['baseline_sha256'] == sha(ROOT/'docs/m5_search_evidence.json'), 'baseline changed')
    ...
    target = dict(tokens_per_second=0.1, boundary='resident START through safe return and token copy',
                  achieved=stats['host_request_through_delivery_seconds']['mean'] <= 10.0)
    require(evidence['target'] == target, 'target/boundary claim changed')
    require(target['achieved'], '0.1 delivered token/s target not attained by this release')
    ...
    for directory in (FINAL, REPRO):
        ...
        derivation = json.loads(read(directory+'/generated/tenth_derivation.json'))
        require(derivation['variant'] == 'reciprocal', 'wrong release variant')
    ...
    for file, marker in (('specialization_test.log', 'M5 TENTH FACTOR EXACT PASS 984736'),
                         ('specialization_test.log', 'M5 TENTH RECIPROCAL EXACT PASS 298048'),
                         ('scaling_test.log', 'M5 TENTH POWER2 EXACT PASS 990'),
                         ('round_test.log', 'M5 TENTH SIGNED ROUND EXACT PASS 321205'),
    ...
    require(read(FINAL+'/unload.log').strip() == 'M5 TENTH NORMAL UNLOAD PASS', 'normal unload missing')
...
def collect():
    ...
    old = json.loads(read('docs/m5_search_evidence.json'))
    evidence = dict(schema=1, status='PASS', baseline_sha256=sha(ROOT/'docs/m5_search_evidence.json'),
    ...
    evidence['target'] = dict(tokens_per_second=0.1, boundary='resident START through safe return and token copy',
                              achieved=evidence['statistics']['final']['host_request_through_delivery_seconds']['mean'] <= 10.0)
    validate(evidence)
    return evidence
...
def main():
    ...
    parser.add_argument('--evidence', default='docs/m5_tenth_evidence.json')
    args = parser.parse_args()
    subprocess.run([sys.executable, '-m', 'scripts.audit_m5_search'], cwd=ROOT, check=True)
    if args.collect:
    ...
    print('M5 TENTH AUDIT PASS: three measured cycles, exact faster firmware, unchanged hardware and safe release')
