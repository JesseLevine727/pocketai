#!/usr/bin/env bash
# User-approved 95 MHz, identical other probe constraints; finite physical trial.
set -euo pipefail
cd "$(dirname "$0")/.."
output=${1:?usage: m6_run_local95_probe.sh build/m6_contract_probe fresh_tag}
...
sha256sum asic/m6/local95_probe_entry.py asic/m6/local95_probe_cts.tcl asic/m6/bank_probe_95.sdc \
   ...
   scripts/m6_run_local95_probe.sh > "$output/${tag}_flow_sources.sha256"
...
timeout --signal=TERM --kill-after=20s 900s docker run --rm --name "$container" \
  ...
  --run-tag "$tag" --from M6Local95.CTS --to OpenROAD.STAPostPNR \
  ...
  -c RUN_POST_GRT_DESIGN_REPAIR=true -c RSZ_CORNERS=max_ss_100C_1v60 \
