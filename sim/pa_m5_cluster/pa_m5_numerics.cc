...
class Simulation {
 public:
  ...
  void tick() {
    require(cycles < 500000000, "global simulation watchdog");
  ...
  void complete_numerics() {
    wait_for([&] { return top.software_done_o; }, 400000000, "dual-Ibex numerical completion");
    stop_and_drain();
    for (unsigned hart = 0; hart < 2; ++hart) {
      std::array<uint32_t, 16> result{};
      for (unsigned i = 0; i < result.size(); ++i)
        result[i] = read_ram(0xd000u + hart * 64u + i * 4u);
      std::printf("M5 IBEX NUMERICS hart=%u state=%u errors=%08x norm=%08x affine=%08x dynamic=%08x round=%08x storage=%08x factor=%u cases=%u cycles=%u cause=%08x address=%08x pc=%08x correction=%08x%08x\n",
                  hart, result[0], result[1], result[2], result[3], result[4], result[5],
                  result[6], result[7], result[8], result[10] - result[9], result[11], result[12], result[13], result[15], result[14]);
      require(result[0] == (hart == 0 ? 3u : 2u) && result[1] == 0 &&
              result[2] == (hart ? 0x783a8841u : 0x96dc8a24u) &&
              result[3] == (hart ? 0x92a2e908u : 0xc712e908u) &&
              result[4] == (hart ? 0xb80bb28eu : 0xdbf0ab4fu) &&
              result[5] == 0x208555c5u && result[6] == 0xe62eb74au &&
              result[7] == (hart ? 2u : 1u) && result[11] == 0,
              "RV32 numerical result mismatch");
    }
  }
...
int main(int argc, char **argv) {
  try {
    ...
    sim.configure(false); sim.load(image); sim.start(); sim.complete_numerics();
    std::printf("M5 ACTUAL IBEX SOFT-F64 NUMERICS PASS cycles=%llu\n",
                static_cast<unsigned long long>(sim.cycles));
    sim.top.final(); return 0;
  } catch (const std::exception &error) {
    std::fprintf(stderr, "M5 ACTUAL IBEX SOFT-F64 NUMERICS FAIL: %s\n", error.what()); return 1;
  }
}
