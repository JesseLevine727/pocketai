...
set build_dir [file normalize $::env(M2_VIVADO_BUILD_DIR)]
set board_repo [file normalize $::env(PYNQ_BOARD_REPO)]
set source_tcl [file normalize $::env(M2_SOURCE_TCL)]
...
create_project m2_pynq $build_dir -part xc7z020clg400-1 -force
...
create_bd_cell -type module -reference pa_cluster_m2_board pa_cluster_m2_0
...
connect_bd_intf_net [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S] \
                    [get_bd_intf_pins pa_cluster_m2_0/S_AXIS_GEMM]
connect_bd_intf_net [get_bd_intf_pins pa_cluster_m2_0/M_AXIS_GEMM] \
                    [get_bd_intf_pins axi_dma_0/S_AXIS_S2MM]

assign_bd_address
set cluster_seg [get_bd_addr_segs -of_objects [get_bd_intf_pins pa_cluster_m2_0/S_AXI_CTRL]]
...
set bd_file "$build_dir/m2_pynq.srcs/sources_1/bd/system/system.bd"
...
set margin_xdc "$build_dir/m2_implementation_margin.xdc"
set margin_fd [open $margin_xdc w]
puts $margin_fd "set m2_margin_clocks \[get_clocks -quiet -filter {PERIOD > 10.52 && PERIOD < 10.54}\]"
puts $margin_fd "set_clock_uncertainty -setup $implementation_margin_ns \$m2_margin_clocks"
close $margin_fd
...
write_checkpoint -force "$build_dir/m2_candidate.dcp"
...
report_utilization -file "$reports_dir/m2_utilization.rpt"
report_timing_summary -file "$reports_dir/m2_timing_summary.rpt"
report_route_status -file "$reports_dir/m2_route_status.rpt"
report_drc -file "$reports_dir/m2_drc.rpt"
report_methodology -file "$reports_dir/m2_methodology.rpt"
report_timing -delay_type max -max_paths 100 -nworst 1 \
    -file "$reports_dir/m2_worst_paths.rpt"

set timing_fd [open "$reports_dir/m2_timing_summary.rpt" r]
...
foreach check {no_clock constant_clock pulse_width_clock
    ...
    if {![regexp $check_pattern $timing_report -> check_count] || $check_count != 0} {
        error "Final timing constraint check failed: $check; inspect m2_timing_summary.rpt"
    }
}

set route_fd [open "$reports_dir/m2_route_status.rpt" r]
set route_report [read $route_fd]
close $route_fd
if {![regexp {# of nets with routing errors\.+\s*:\s*([0-9]+)} $route_report -> route_errors] || $route_errors != 0} {
    error "Final design is not fully routed; inspect m2_route_status.rpt"
}
set methodology_fd [open "$reports_dir/m2_methodology.rpt" r]
...
set bit_src "$build_dir/m2_pynq.runs/impl_1/system_wrapper.bit"
write_checkpoint -force "$build_dir/m2_pynq_final.dcp"
write_bitstream -force $bit_src
file copy -force $bit_src "$build_dir/m2_pynq.bit"
set hwh_src "$build_dir/m2_pynq.gen/sources_1/bd/system/hw_handoff/system.hwh"
if {![file exists $hwh_src]} {
    error "PYNQ hardware handoff missing: $hwh_src"
}
file copy -force $hwh_src "$build_dir/m2_pynq.hwh"
write_hw_platform -fixed -include_bit -force -file "$build_dir/m2_pynq.xsa"
set preferred_margin_met [expr {$worst_slack >= 0.500}]
puts "M2 VIVADO PASS clock_mhz=$target_clock_mhz setup_wns_ns=$worst_slack setup_tns_ns=0 min_setup_slack_ns=$minimum_setup_slack hold_wns_ns=$worst_hold_slack dsp48=0 drc_errors=0 reviewed_drc_warnings=[llength $drc_warnings] preferred_margin_0p500ns=$preferred_margin_met"
